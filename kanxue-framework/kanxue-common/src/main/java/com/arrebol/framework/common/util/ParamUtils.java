package com.arrebol.framework.common.util;
...
public final class ParamUtils {
     ...
     * 看雪 ID 校验
     */
    public static boolean checkKanxueId(String kanxueId) {
        // 检查长度
        if (kanxueId.length() < ID_MIN_LENGTH || kanxueId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(kanxueId).matches();
    }
